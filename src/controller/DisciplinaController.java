...
public class DisciplinaController implements ActionListener{
	...
	private void salvarDisciplina() throws Exception {
	    ...
	    if (!CursoController.controladorPrincipal.cursoExiste(codCursoDigitado)) {
	        taDisciplina.setText("ERRO: O curso " + codCursoDigitado + " não está cadastrado. A disciplina não foi salva.");
	        return; 
	    }
	    ...
	    if (this.disciplinaEmEdicao != null) {
	        ...
	        if (!this.disciplinaEmEdicao.getCod().equals(codDigitado)) {
	            taDisciplina.setText("ERRO: O código da disciplina (" + this.disciplinaEmEdicao.getCod() + ") não pode ser alterado após a busca.");
	        ...
	        codProcesso++;
	        filaDisciplinas.insert(novaDisciplina);
	        if (listaDisciplinas.isEmpty()) {
