...
public class InscricaoController implements ActionListener{
	...
	private Fila<Inscricao> filaInscricoes = new Fila<>();
	...
	public InscricaoController(JTextField CPF, JTextField codDisciplina, JTextField codProcesso, JTextField inscricaoBuscar, JTextArea taInscricao) {
		tfInscricaoCPF = CPF;
		tfInscricaoCodDisciplina = codDisciplina;
		...
		try {
			...
			for (int j = 0; j < tamanho; j++) {
				...
				if (i.getCPF().equals(busca)) {
	                ...
	                tfInscricaoCodDisciplina.setText(i.getCodDisciplina());
		...
		private void salvarInscricao() throws Exception {
			if (this.inscricaoEmEdicao != null) {
		        ...
		        if (!this.inscricaoEmEdicao.getCPF().equals(cpfDigitado)) {
		            taInscricao.setText("ERRO: O CPF da inscrição (" + this.inscricaoEmEdicao.getCPF() + ") não pode ser alterado.");
		            tfInscricaoCPF.setText(this.inscricaoEmEdicao.getCPF()); 
		            return;
		        }
		        inscricaoEmEdicao.setCodDisciplina(tfInscricaoCodDisciplina.getText());
		    ...
		    for (int j = 0; j < tamanho; j++) {
		        ...
		        if (insc.getCPF().equals(cpfDigitado) && insc.getCodProcesso().equals(codProcessoDigitado)) {
		            taInscricao.setText("ERRO: O professor " + cpfDigitado + " já está inscrito no processo " + codProcessoDigitado + ".");
		            return;
		        }
		    }
		    tfInscricaoCodDisciplina.setText(d.getCod());
		    Inscricao i = new Inscricao();
		    ...
		    taInscricao.setText("Inscrição do CPF " + i.getCPF() + " no processo " + i.getCodProcesso() + " salva com sucesso!");
		    limparCampos();
		}
